...
-- ============================================================
--  FILE 2: PL/SQL – Procedures, Functions, Cursors,
--           Triggers, Exception Handling
--
--  NOTE: Written for Oracle SQL syntax.
--        Run in Oracle SQL Developer or Oracle Live SQL.
--        SET SERVEROUTPUT ON;   -- run first to see output
-- ============================================================
...
-- SELECT get_total_bill(1) AS Total_Bill FROM DUAL;
...
-- SELECT get_doctor_appt_count(1) AS Appointment_Count FROM DUAL;
...
BEGIN
    ...
    FOR rec IN cur_doctors LOOP
        DBMS_OUTPUT.PUT_LINE(
            rec.full_name || ' (' || rec.specialization || ')' ||
            '  =>  Appointments: ' || rec.appt_count
        );
    END LOOP;
END;
/
...
BEGIN
    ...
    IF v_count > 0 THEN
        RAISE_APPLICATION_ERROR(
            -20010,
            'Cannot delete Patient ID ' || :OLD.patient_id ||
            ' – ' || v_count || ' appointment(s) exist.'
        );
    END IF;
END;
/
